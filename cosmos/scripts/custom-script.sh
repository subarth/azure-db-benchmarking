...
PARTITION_COUNT=${PARTITION_COUNT:-1000}
if ! [[ "$PARTITION_COUNT" =~ ^[1-9][0-9]*$ ]]; then
  echo "PARTITION_COUNT must be a positive integer"
  exit 1
fi
...
git pull
git apply --ignore-space-change /tmp/ycsb/ycsb-partitioned-scan.patch || exit 1
